...
%Columns are: case, herring biomass, cod biomass, cod numbers. 
INITCOND= ...
    [1 9.28	2.73	 336.15
    2 1.43	2.73	 336.15
    3 9.29	0.77	 82
...
%% Generating the initial guess for the solutions using tau=0 and
% independent system.
...
% Generating initial guess for the different ICs.
for CASE=1:4;
    cbnd = initial({x1==INITCOND(CASE,2)
        x2==INITCOND(CASE,3)
        n2==INITCOND(CASE,4)});
    % Code for including a time delay in the state equations -- see Tomlab
    % manual
...
%% Solutions for different models analysis
for FLAG=1:4 % different ecological models
    ...
    for CASE=1:4  % Initial condition cases
        ...
        for  ii=[3 5 7] % currently code is written in a fixed way assuming you start at the 3rd node
            ...
            for STARTING=1:2
               ...
               % In this first runs, we are not using the time lag as a
               % means to generate good initial guesses for the solution
                ...
                % Reads this out to the screen to keep track of the runs
                options.name = ['Dyanmics: Flag=' num2str(FLAG) ': CASE=' num2str(CASE) ':Starting=' num2str(STARTING) ': CollocationNodes=' num2str(ii)]
                options.solver = 'snopt';
                
                
                % This loop was a way to keep refining the initial guess
                % for the different numbers of collocation nodes. It
                % proved robust across the different parameter settings and
                % ecological models. 
                ...
                elseif ii==3 && STARTING~=1
                    ...
                    % another way to call the solver that permits more
                    % flexibility with respect to parameters in  the
                    % algorithm
                    
                    % This is final run for the paper
        ...
        %but are kept due to code below that was changed at a later date
        %for the T analysis only). 
